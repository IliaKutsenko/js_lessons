...
function f3() {
    let divName = document.getElementsByClassName('out-3');
    for (let i = 0; i < divName.length; i++) {
        divName[i].innerHTML = 3;
    }
}
...
function f8() {
    let result = [];
    for (let obj in a8) {
        if (typeof a8[obj].id == 'number') {
            result[obj] = a8[obj].id;
        }
    }
    return result;
}
...
function f15() {
    let node = document.querySelectorAll('.out-15');
