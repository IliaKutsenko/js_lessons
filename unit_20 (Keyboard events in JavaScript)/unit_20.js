...
function t7(event) {
    let t = Math.floor((Math.random() * (a7.length - 0) + 0));
    event.key += a7[t];
    document.querySelector('.out-7').innerHTML += a7[t];
}
...
function t8(event) {
    ...
    for (let key in a8) {
        if (key == t) {
            document.querySelector('.out-8').innerHTML += a8[t];
        }
    }
...
function t9(event) {
    ...
    if (event.key == 'ArrowDown') {
        n9++;
        document.querySelector('.out-9').innerHTML = n9;
    }
}
...
function t11(event) {
    ...
    let kb = document.getElementsByClassName('keyboard');
    for (let i = 0; i < kb.length; i++) {
        let data = kb[i].getAttribute('data');
        if (kb[i].classList.contains('active')) {
            kb[i].classList.remove('active');
        }
        if (event.key.toLowerCase() == data.toLowerCase()) {
            kb[i].classList.add('active');
        }
    }
}
