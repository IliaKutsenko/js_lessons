...
function t1() {
    ...
    xhttp.open("GET", 'http://getpost.itgid.info/index2.php?auth=37A38D7d5b44Dd6D856478D8&action=1');
    xhttp.send();
...
function t10() {
    ...
    let qstr = new URLSearchParams(arr);
    let paramsOnly = qstr.toString();
    let paramsUrl = 'http://getpost.itgid.info/index2.php?' + qstr.toString();
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            myFunc(this.responseText);
        }
    }
    xhttp.open("POST", paramsUrl);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send(paramsOnly);
    function myFunc(data) {
